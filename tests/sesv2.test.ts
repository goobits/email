...
import { createSesV2Provider } from '../src/providers/sesv2.ts'

function createFakeSesV2Client(responder: (command: unknown) => unknown | Promise<unknown>) {
	return {
		send: vi.fn(async (command: unknown) => responder(command))
	} as unknown as Parameters<typeof createSesV2Provider>[0]['client']
}

describe('createSesV2Provider', () => {
	it('uses SendEmailCommand with simple text/html content', async () => {
		let captured: any
		const client = createFakeSesV2Client(command => {
			captured = command
			return { MessageId: 'sesv2-abc-123' }
		})
		const provider = createSesV2Provider({ client })
		...
		if (result.success) {
			expect(result.provider).toBe('aws-ses-v2')
			expect(result.messageId).toBe('sesv2-abc-123')
		}
	...
	it('forwards cc, bcc, replyTo, and configurationSetName', async () => {
		...
		const provider = createSesV2Provider({ client, configurationSetName: 'tx-emails' })
		await provider.send({
			from: 'sender@example.com',
			to: 'user@example.com',
			cc: [ 'cc@example.com' ],
	...
	it('maps attachments to SES v2 Simple.Attachments', async () => {
		let captured: any
		const client = createFakeSesV2Client(command => {
			captured = command
			return { MessageId: 'x' }
	...
	it('maps custom headers to SES v2 message headers', async () => {
		let captured: any
		const client = createFakeSesV2Client(command => {
			captured = command
			return { MessageId: 'x' }
	...
	it('rejects invalid custom headers before calling SES v2', async () => {
		const client = createFakeSesV2Client(() => ({ MessageId: 'x' }))
		const provider = createSesV2Provider({ client })
		const result = await provider.send({
			from: 'sender@example.com',
			to: 'user@example.com',
	...
	it('returns configuration-missing when from is omitted', async () => {
		const provider = createSesV2Provider({ client: createFakeSesV2Client(() => ({ MessageId: 'x' })) })
		const result = await provider.send({
			to: 'u@e.com',
			subject: 'x',
			text: 'x'
	...
	it('maps SES v2 throttling errors to rate-limited reason', async () => {
		const client = createFakeSesV2Client(() => {
	...
	it('maps SES v2 rejected messages to invalid-recipient reason', async () => {
		const client = createFakeSesV2Client(() => {
	...
	it('verify() returns success when client present', async () => {
		const provider = createSesV2Provider({ client: createFakeSesV2Client(() => ({})) })
